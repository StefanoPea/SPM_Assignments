...
static inline void usage(const char *argv0) {
    ...
    std::printf(" -B <block size> override parallel chunk size in bytes (default B=%zu)\n", CHUNK_SIZE);
    std::printf("--------------------\n");
}

int parseCommandLine(int argc, char *argv[]) {
    extern char *optarg;
    const std::string optstr = "r:C:D:q:B:";
    long opt, start = 1;
    ...
    while ((opt = getopt(argc, argv, optstr.c_str())) != -1) {
        switch (opt) {
            ...
            case 'B': {
                long b = 0;
                if (!isNumber(optarg, b) || b <= 0) {
                    std::fprintf(stderr, "Error: wrong '-B' option\n");
                    usage(argv[0]);
                    return -1;
                }
                b = b * 1024 * 1024; // Convert to megabytes
                CHUNK_SIZE = static_cast<size_t>(b);
                start += 2;
            } break;
